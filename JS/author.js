...
////////// Müəllif adını yaratmaq üçün funksiya 

function printAuthors(author) {
    return  `<p class="text-red-500 hover:text-red-700 cursor-pointer text-sm">${author}</p>`
};

// Hərflərin qruplanması və göstərilməsi 

async function getAuthorsData() {
    try {
         const data = await getAllBooks(); 
         
        //  Müəlliflərin adlarını çıxarılması və əlifba sırası ilə sıralanması 

        const authors = [...new Set(
            data.filter(book => book.author?.trim())
            .map(book => book.author.trim())
        )].sort((a, b) => a.localeCompare(b, 'az'));


        // Hərflərin qruplanması 

        const grouped = authors.reduce((groups, author) => { 
            ...
            <div class="w-full max-w-6xl mx-auto p-4">
                <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8">
                    ${Object.keys(grouped).sort().map(letter => `
                        <div>
                            <h3 class="text-xl font-bold text-gray-800 mb-3 border-b pb-1">${letter}</h3>
                            <div class="space-y-1">
                                ${grouped[letter].map(printAuthors).join('')}
                            </div>
                        </div>
                    `).join('')}
                </div>
        ...
        document.getElementById("authorContainer").innerHTML = `<div class="text-red-600 text-center">Xəta baş vedi</div>`  
    };
};
