...
function libraffPrintBook() { 
  try {
    ...
    libraffBooks.forEach(book => {
        ...
        <div class="max-w-72 h-[70dvh]  rounded-md shadow-md cursor-pointer">
              <img src="${book.img}" alt="${book.name}" title="${book.name}" class="object-cover w-full   h-[70dvh]  rounded-md  dark:bg-gray-500">
        </div>
        `
    })
  } catch (error) {
